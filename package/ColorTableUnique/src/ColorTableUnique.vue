<template>
    <div>
        ...
        <modal v-model="show" title="颜色配置" :width="460" :mask-closable="false" @on-ok="ok" @on-cancel="cancel">
            <div>
                <div class="lineheader">
                    <div class="columns">状态值</div>
                    <div class="columns">颜色</div>
                    <div class="action">操作</div>
                </div>
                <div class="line" v-for="(col,index) in form" :key="index">
                    <div class="columns">
                        <i-input v-model="col.value" style="width:114px" />
                    </div>
    ...
    methods: {
        ...
        addline() {
            if (this.isgradientRamp) {
                ...
                this.form.push({ value: '', color: '#FFFFFF' });
            }
        },
        deletelline(index) {
        ...
        changeColorFormat(val, index) {
            if (val.includes('rgba')) {
                this.form[index].color = this.rgbaToHex(val);
            }
        },
        // 转换数据
        ...
        changeSingleColor(v, index) {
            ...
            if (this.form[index].colorGradient.classification === 'single') {
                if (RGBAObj.rgba) {
                    const RGBAStr = `rgba(${RGBAObj.rgba.r},${RGBAObj.rgba.g},${RGBAObj.rgba.b},${RGBAObj.rgba.a})`;
                    this.form[index].colorGradient.data.type = 'single';
                    this.form[index].colorGradient.data.single.value = RGBAStr;
                    // = this.selectColors.rgba = RGBAStr;
                } else if (RGBAObj.indexOf('rgba') !== -1) {
                    this.form[index].colorGradient.data.type = 'single';
                    this.form[index].colorGradient.data.single.value = RGBAObj;
                }
                // this.$parent.setColorPS(this.selectColors);
            }
        },
        changeLinearColor(object, index) {
            console.log(this.form, index, object);
            this.form[index].colorGradient.data.type = 'single';
            this.form[index].colorGradient.data.single.value = object.color;
        },
        colorPickConfirm(index) {
            const val = this.form[index].colorGradient.data.single.value;
            if (val && val.constructor === Object) {
                ...
                this.form[index].color = {
                    values: [],
                    intervals: [],
                    gradientType: '',
                };
                arr.forEach((item) => {
                    this.form[index].color.values.push(map.get(item));
                });
                this.form[index].color.intervals = arr;
                this.form[index].color.gradientType = 'vertical';
            } else {
                this.form[index].color = this.rgbaToHex(val);
            }
        },
        colorPickCancel(index) {
            this.form[index].colorGradient.data.single.value = JSON.parse(JSON.stringify(this.form[index].color));
        },
